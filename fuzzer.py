...
class ValueGenerator():
    def __init__(self, value_type, strings=True):
        ...
        global values_16bit
        global values_32bit
        self.values = values_strings if strings else [] # self.values has all the possible values
        if value_type == 'byte':
...
class Mutator():
    def __init__(self, original_file, tmp_directory, value_type):
        ...
        self.original_file_base = os.path.split(original_file)[-1]
        self.original_file_base = os.path.splitext(self.original_file_base)[0]    # yes... i called this twice for asthetic values
        self.original_file_ext  = os.path.splitext(original_file)[1]
        ...
        try:
            ...
            print 'Unable to open', self.original_file
            exit()
        ...
        for offset in range(self.original_bytes_len):
            for value in self.value_generator.values:
                ...
                if value['type'] == 'replace':
                    new_bytes[offset:offset+value['size']] = value['value']
                elif value['type'] == 'insert':
                    new_bytes = new_bytes[:offset] + value['value'] + new_bytes[offset:]
                else:
...
class MyEventHandler (EventHandler):
    ...
    def handleEvent(self, s, event):
        ...
        print r'glob.glob(C:\DocumentsandSettings\Administrator\Desktop\fuzzing\files\) = %s' % (glob.glob(r'C:\DocumentsandSettings\Administrator\Desktop\fuzzing\files\*'))
...
def monitor_thread(cmd_line, offset, value, value_type, new_file, run_number):
    ...
    output = '%s %d %s\n' % ('-'*10, run_number, '-'*10)
    output += 'Running : %s\n' % cmd_line
    output += 'run number = %d, offset = %d, value = %s, value_type = %s' % (run_number, offset, repr(value), value_type)
    if handler.output:
